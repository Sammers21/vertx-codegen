...
public class CodeGen {
  ...
  private final HashMap<String, TypeElement> webApiProxyClasses = new HashMap<>();
  ...
  public CodeGen(ProcessingEnvironment env, RoundEnvironment round, ClassLoader loader) {
    ...
    round.getElementsAnnotatedWith(WebApiProxyGen.class).
      stream().
      filter(implFilter).
      forEach(element -> webApiProxyClasses.put(Helper.getNonGenericType(element.asType().toString()), (TypeElement) element));
  }
  ...
  public Stream<Map.Entry<TypeElement, ProxyModel>> getWebApiProxyModels() {
    return webApiProxyClasses.entrySet().stream().map(entry -> new ModelEntry<>(entry.getValue(), () -> getWebApiProxyModel(entry.getKey())));
  }
  ...
  public WebApiProxyModel getWebApiProxyModel(String fqcn) {
    TypeElement element = webApiProxyClasses.get(fqcn);
    if (element == null) {
      throw new IllegalArgumentException("Source for " + fqcn + " not found");
    } else {
      WebApiProxyModel model = new WebApiProxyModel(env, methodOverloadChecker, messager, classes, elementUtils, typeUtils, element);
      model.process();
      return model;
    }
  }
